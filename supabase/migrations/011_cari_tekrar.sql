-- ============================================================
-- PilotGarage — 011: Recurring cari hareketler (owner request 2026-07-09)
--
-- İşletme Detay's Gelir/Gider Ekle gains the same "otomatik ödeme günü"
-- picker as the Finans modal. A rule that targets a cari işletme is
-- materialized by the daily cron as a new cari HAREKET (kasa_durumu =
-- 'YOK') — NOT as a kasa işlemi. The money invariants are untouched:
-- the auto-created hareket still needs "Kasaya Yansıt" and then Onay
-- before it can reach the kasa.
-- ============================================================

-- A rule may target a cari işletme; its rules die with it.
alter table public.tekrar_kurallari
  add column cari_isletme_id uuid references public.cari_isletmeler (id) on delete cascade;

-- Trace + cron-rerun dedupe (same pattern as islemler' tekrar index).
alter table public.cari_hareketler
  add column tekrar_kural_id uuid references public.tekrar_kurallari (id) on delete set null;

create unique index cari_hareketler_tekrar_ux
  on public.cari_hareketler (tekrar_kural_id, tarih)
  where tekrar_kural_id is not null;

-- ── Materializer: cari-targeted rules produce hareketler ──
